...
import { Button, Chip, Image, Spinner, Tooltip, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure, Input, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Checkbox } from "@heroui/react";
...
export default function BehaviorPacksPage() {
  ...
  const [sortKey, setSortKey] = React.useState<"name" | "time">(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("content.behavior.sort") || "{}");
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem("content.behavior.sort", JSON.stringify({ sortKey, sortAsc }));
    } catch {}
  }, [sortKey, sortAsc]);
      ...
      >
        ...
        <div className="rounded-xl border border-default-200 bg-white/50 dark:bg-neutral-800/40 shadow-sm backdrop-blur-sm px-3 py-2 mt-4 flex items-center justify-end">
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="bordered" onPress={() => OpenPathDir(roots.behaviorPacks)}>
                ...
                {packs.length ? (
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                    {(() => {
                      ...
                      <div
                        key={`${p.path}-${idx}`}
                        className={`relative rounded-xl border p-3 transition-colors h-44 ${selectMode ? 'cursor-pointer' : 'cursor-default'} ${selectMode && selected[p.path] ? 'border-primary-300 dark:border-primary-400 bg-primary/10 dark:bg-primary/15 shadow-sm' : 'border-default-200 bg-white/70 dark:bg-neutral-800/50 hover:bg-white/80 dark:hover:bg-neutral-800/60'}`}
                        onClick={() => {
                          if (!selectMode) return;
                          setSelected((prev) => ({ ...prev, [p.path]: !prev[p.path] }));
                        }}
                      >
                        <div className="flex items-start gap-3 h-full">
                          {selectMode ? (<div onClick={(e) => e.stopPropagation()}><Checkbox size="sm" isSelected={!!selected[p.path]} onValueChange={() => setSelected((prev) => ({ ...prev, [p.path]: !prev[p.path] }))} className="shrink-0" /></div>) : null}
                          {p.iconDataUrl ? (
